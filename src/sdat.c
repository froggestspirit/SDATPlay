...
unsigned char * load_file(const char *filename){
    ...
    //printf("File: %s\n",filename);
    if (0!=fseek(file, 0, SEEK_END)) return false;
	int sizef=ftell(file);
	if (0!=fseek(file, 0, SEEK_SET)) return false;
    unsigned char *buffer = (unsigned char *)malloc(sizef);
	if (sizef!=fread(buffer, 1, sizef, file)) return false;
	//printf("File Size: %X\n",sizef);
	fclose(file);
    return buffer;
}
...
void readTemp(int bytes, unsigned char *source){
    for(int ini=0; ini<bytes; ini++){
        temp[ini]=source[filePos++];
    }
}

void readTempKey(int bytes, unsigned char *source){
    for(int ini=0; ini<bytes; ini++){
        tempKey[ini]=source[filePos++];
    }
}
...
bool NDS_decInst(int index){//decode samples to a buffer for faster playback (mostly for rasp-pi zero)
    ...
    if(instType[index]==0){//blank
        ...
        filePos=instAddress[index];
        readTempKey(2, sdat);
        printf("%d-%d",tempKey[0], tempKey[1]);
    ...
    for(int sampI=0; sampI<sampleEnd[ID]; sampI++){
        if(sampleFormat[ID]==2){///IMA-ADPCM
            if(sampleNibbleLow){
                ...
                if (temp[0]&8){samplePredictor=fmax(samplePredictor-sampleDiff,-0x7FFF);}
                else{samplePredictor=fmin(samplePredictor+sampleDiff,0x7FFF);}
                sampleStepIndex=fmax(fmin(sampleStepIndex+INDEX_TABLE[temp[1]],88),0);
                sampleNibbleLow=false;
            }else{
                temp[1] = tempNibble & 0x70;//delta
                sampleDiff = ADPCM_TABLE[sampleStepIndex+(temp[1]<<3)];
                if (tempNibble&0x80){samplePredictor=fmax(samplePredictor-sampleDiff,-0x7FFF);}
                else{samplePredictor=fmin(samplePredictor+sampleDiff,0x7FFF);}
                sampleStepIndex=fmax(fmin(sampleStepIndex+INDEX_TABLE[temp[1]>>4],88),0);
                sampleNibbleLow=true;
                samplePos[0]++;
            }
            ...
            decSWAVBuffer[freeSpace+sampI] = (samplePos[0])<((sampleFormat[ID]-2)<<2) ? 0x7FFF/(float)(0x080000) : 0x0000/(float)(0x080000);
            samplePos[0]++;
        }
    }
...
int decode_swav(int ID, int freeSpace, const char *swav){
    int swav_id = (int)(swav[0]) - 48;
    unsigned char *swavData;
    if(swav_id >= 0 && swav_id<= 9){  //PSG
        sampleFormat[ID]=3+swav_id;//store the PSG duty here
        sampleLoops[ID]=true;
        samplePitch[ID]=0x20B7;//This isn't divisible by 4, so we multiply the length of the 8-bit waveforms by 4
        sampleLoop[ID]=0;
        sampleLoopLength[ID]=32;//8*4=32
        ...
        sampleLoopLength[ID]=(temp[3]<<26)+(temp[2]<<18)+(temp[1]<<10)+(temp[0]<<2);
        sampleEnd[ID]=sampleLoop[ID]+sampleLoopLength[ID];
        sampleOffset[ID]=0x24;//skip the header
        if(sampleFormat[ID]==1){
            sampleEnd[ID]/=2;
            sampleLoop[ID]/=2;
            sampleLoopLength[ID]/=2;
...
void set_swav(int index, int key, const char *swav){
    int keyiPointer = (index << 7) + key;
    decSWAVPointer[keyiPointer] = decSWAVFree;
    decSWAVFree = decode_swav(keyiPointer, decSWAVFree, swav);
}

void set_swav_range(int index, int keyFrom, int keyTo, const char *swav){
    set_swav(index, keyTo, swav);

    int keyiPointer = (index << 7);
    int goalKeyPointer = keyiPointer + keyTo;
    for(int i = keyFrom + keyiPointer; i < keyTo + keyiPointer; i++){
        decSWAVPointer[i] = decSWAVPointer[goalKeyPointer];
        sampleFormat[i]=sampleFormat[goalKeyPointer];
        sampleLoops[i]=sampleLoops[goalKeyPointer];
        samplePitch[i]=samplePitch[goalKeyPointer];
        sampleLoop[i]=sampleLoop[goalKeyPointer];
        sampleLoopLength[i]=sampleLoopLength[goalKeyPointer];
        sampleEnd[i]=sampleEnd[goalKeyPointer];
    }
}

void set_inst(int index, int key, const char *swav, int note, int attack, int decay, int sustain, int release, int pan){
    int keyiPointer = (index << 7) + key;
    set_swav(index, key, swav);
    keyAttack[keyiPointer] = attack;
    keyDecay[keyiPointer] = decay;
    keySustain[keyiPointer] = sustain;
    keyRelease[keyiPointer] = release;
    keyPan[keyiPointer] = pan;
    keyRootVal[keyiPointer]=note;
    keyRoot[keyiPointer]=FREQ_TABLE[(note<<7)];
}
...
void set_inst_range(int index, int keyFrom, int keyTo, const char *swav, int note, int attack, int decay, int sustain, int release, int pan){
    set_inst(index, keyTo, swav, note, attack, decay, sustain, release, pan);
    int keyiPointer = (index << 7);
    int goalKeyPointer = keyiPointer + keyTo;
    for(int i = keyFrom + keyiPointer; i < keyTo + keyiPointer; i++){
        keyAttack[i] = attack;
        keyDecay[i] = decay;
        keySustain[i] = sustain;
        keyRelease[i] = release;
        keyPan[i] = pan;
        keyRootVal[i]=note;
        keyRoot[i]=keyRoot[goalKeyPointer];
        decSWAVPointer[i] = decSWAVPointer[goalKeyPointer];
        sampleFormat[i]=sampleFormat[goalKeyPointer];
        sampleLoops[i]=sampleLoops[goalKeyPointer];
        samplePitch[i]=samplePitch[goalKeyPointer];
...
void load_sseq(const char *filename){
    ...
    for(int i=1; i<16; i++){
        if((usedTracks&(1<<i))!=0){
            temp[0]=sseq[filePos++];//read 0x93 channel pointer command 
            readTemp(4, sseq);//read channel number and pointer
...
bool sdat_load_song(const char *filename, int songID){
    sseq = sdat = load_file(filename);
    //Read SDAT header
    filePos=0x0E;
    readTemp(2, sdat);
    printf("numBlocks:%X\n",temp[0]);
    ...
    for(int i=1; i<16; i++){
        if((usedTracks&(1<<i))!=0){
            ...
            readTemp(4, sdat);//read channel number and pointer
            chPointer[temp[0]]=(temp[3]<<16)+(temp[2]<<8)+temp[1]+dataOffset;
            chPointer[0]+=5;
            chActive[temp[0]]=true;
            timesLooped[temp[0]]=0;
    ...
    printf("\nDone!\n");
    printf("Insst Buffer:0x%X/0x4000000\n",(decSWAVFree*2));
    //read swar header
    chActive[0]=true;
                            ...
                            default: //
                                if(temp[0]<0x80){//Note on
                                    ...
                                    if(curSlot<0xFF){
                                        ...
                                        fmax(slotPanL[curSlot]=chPanL[i]+PAN_TABLE[slotPan[curSlot]],-723);
                                        fmax(slotPanR[curSlot]=chPanR[i]+PAN_TABLE[127-slotPan[curSlot]],-723);
